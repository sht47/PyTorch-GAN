...
def main():
    ...
    trainA_data = datasets.ImageFolder(root='./cad2real_A', transform=data_transform)
    trainB_data = datasets.ImageFolder(root='./cad2real_B', transform=data_transform)
    trainA_data_loader = DataLoader(trainA_data, batch_size=opt.batch_size, shuffle=True)
    trainB_data_loader = DataLoader(trainB_data, batch_size=opt.batch_size, shuffle=True)
    ...
    for epoch in range(opt.n_epochs):
        for i, ((imgs1, _), (imgs2, _)) in enumerate(zip(trainA_data_loader, trainB_data_loader)):
            ...
            save_checkpoint({
                'epoch': epoch+1,
                'state_dict': coupled_generators.state_dict(),
                'best_mae_error': d_loss,
            }, is_best=True, output_dir='results')
            print(
                "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]"
                % (epoch, opt.n_epochs, i, len(trainA_data_loader), d_loss.item(), g_loss.item())
            )
    
            batches_done = epoch * len(trainA_data_loader) + i
...
# def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    torch.save(state, osp.join(output_dir, filename))
    if is_best:
        shutil.copyfile(osp.join(output_dir, filename), osp.join(output_dir, 'model_best.pth.tar') )
